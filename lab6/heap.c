...
int VerifyHeap(Heap * h) {
    ...
    for (i = 0; i < (h->n_elements - 2)/2; i++) {
        ...
        if(h->heapdata[2 * i + 1] > h->heapdata[i] || h->heapdata[2 * i + 2] > h->heapdata[i])
            return 0;
    }
...
void fixDown(Heap *h, int idx) {
	...
	while (2 * idx < h->n_elements - 1) {
        if(h->heapdata[2 * idx + 1] < h->heapdata[idx])
            child = 2 * idx + 1;
        else
		    child = 2 * idx + 2;
		
        if(h->heapdata[idx] < h->heapdata[child]) {
            aux = h->heapdata[idx];
            h->heapdata[idx] = h->heapdata[child];
            h->heapdata[child] = aux;
        }
...
void Heapify(Heap * h) {
    ...
    for(aux = (h->n_elements - 2)/2 ; aux >= 0; aux--)
        fixDown(h, aux);
